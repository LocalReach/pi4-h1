...
def run(model: str, camera_id: int, width: int, height: int, num_threads: int,
    ...
    while cap.isOpened():
        ...
        elif abs(timer - time.time()) >= 5:
            channel_feed = False
            adbreak = True
        
        print(channel_feed, " ", num_detections, " ", time.time() - timer)
        
        # if channel_feed:
        #     # image = cv2.putText(image, "TSN", (50,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
        # else:
